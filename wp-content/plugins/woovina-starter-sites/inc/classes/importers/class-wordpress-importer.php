...
require WOOVINA_SITES_DIR . 'inc/classes/importers/class-parsers.php';
...
class WP_Import extends WP_Importer {
	...
	function import_options() {
		$j = 0;
?>
<form action="<?php echo admin_url('themes.php?import=wordpress&amp;step=2'); ?>" method="post">
...
<?php
	...
	function fetch_remote_file($url, $post) {
		...
		$headers = WooVina_Demos_Helpers::wp_get_http($url, $upload['file']);
		...
		if(isset($headers['content-length']) && $filesize != $headers['content-length'] && $upload['type'] != 'image/svg+xml') {
			@unlink($upload['file']);
	...
	function greet() {
		...
		wp_import_upload_form('themes.php?import=wordpress&amp;step=1');
		echo '</div>';
	}
